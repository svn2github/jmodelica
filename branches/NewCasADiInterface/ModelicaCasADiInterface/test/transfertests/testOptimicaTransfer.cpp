...
int main(int argc, char *argv[])
{
    ...
    { // User defined types
        ...
        vector< Ref<Variable> > diffs =  optProblem->getModel()->getVariables(Model::DIFFERENTIATED);
        assert( diffs[0]->getDeclaredType().getNode() != NULL );
        assert( diffs[0]->getDeclaredType() == optProblem->getModel()->getVariableType("Voltage"));
    }
