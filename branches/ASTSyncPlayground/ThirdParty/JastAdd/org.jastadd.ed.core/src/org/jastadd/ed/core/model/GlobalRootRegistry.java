...
/**
 * This class may be used as is or be extended via a subclasses. 
 * In either case it should be used as a singleton for one or several plug-ins.
 * 
 * @author emma
 *
 */
public abstract class GlobalRootRegistry implements IGlobalRootRegistry {
	...
	/**
	 * Updates the AST corresponding to the given project.
	 * This method update will try to update as few nodes as possible. The old node entry will be
	 * traversed and compared to the new node.
	 * 
	 * @param project The projects which corresponding AST should be updated
	 * @param newNode The node with new information
	 ...
	 * the new node.
	 * 
	 * @param file The file for which the corresponding AST should be updated
	 * @param newNode The node with new information
	 ...
	 * @param project The project for which the corresponding AST should be discarded
	 * @return true if an AST was found and discarded, or false if no AST was found
	 */
	 ...
	 * @param file The file for which the corresponding AST should be discarded
	 * @return true if file was found and discarded, or false if no file was found
	 */
	 ...
	 * @param project The project to use as key
	 ...
	 * @param file The file to use as key.
	 ...
	 * @param file The node to look up
	 * @return The file node, or null if no entry was found
	 */
	/*
	 * Looks up the project for the node and asks its corresponding 
	 * project node (global root node) for a matching file node 
	 * (local root node).
	 */
	 ...
	 * @param file The file to discard
	...
	protected boolean discardFile(IFile file) {
		...
		try {
			...
			for (IProject p : fProjectASTMap.keySet()) {
				if (project.equals(p)) {
					IGlobalRootNode projectNode = fProjectASTMap.get(file.getProject());
					...
					for (ILocalRootNode fileNode : nodeList) {
						...
						ASTDelta delta = new ASTDelta(fileNode, new IASTDelta[] {});
						delta.setStatus(IASTDelta.REMOVED);
						deltaList.add(delta);
					}
					ASTDelta projectDelta = new ASTDelta(projectNode, deltaList.toArray(new IASTDelta[]{}));
					projectDelta.setStatus(IASTDelta.CHANGED|IASTDelta.CHILD_CHANGED);
					ASTChangeEvent evt = new ASTChangeEvent(ASTChangeEvent.POST_REMOVE, 
							ASTChangeEvent.FILE_LEVEL, projectNode, projectDelta);
					notifyListeners(evt);
					result = true;
				}
			}	
	...
	protected boolean discardProject(IProject project) {
		...
		if (fProjectASTMap.containsKey(project)) {
			...
			ASTChangeEvent evt = new ASTChangeEvent(ASTChangeEvent.POST_REMOVE, 
					ASTChangeEvent.PROJECT_LEVEL, projectNode, projectDelta);
			notifyListeners(evt);
			result = true;
		}
		return result;
	 ...
	 * @param newNode The node to update with
	 * @return true if ok, or false if no enclosing project node was found
	 */
		...
		try {
			...
			for (IProject p : fProjectASTMap.keySet()) {
				if (project.equals(p)) {
					...
					List<ILocalRootNode> nodeList = projectNode.lookupFileNode(file);
					for (ILocalRootNode oldNode : nodeList) {
						if (oldNode.correspondsTo(newNode)) {
							...
							if(oldNode.shouldBeUpdatedWith(newNode)) {
								...
								ASTDelta projectDelta = new ASTDelta(projectNode, childDelta);
								projectDelta.setStatus(IASTDelta.CHANGED|IASTDelta.CHILD_CHANGED);
								ASTChangeEvent evt = new ASTChangeEvent(ASTChangeEvent.POST_UPDATE, 
										ASTChangeEvent.PROJECT_LEVEL, projectNode, projectDelta);
								notifyListeners(evt);
								result = true;								
							}
					...
					if (!foundCorresponding) {
						...
						projectDelta.setStatus(IASTDelta.CHANGED|IASTDelta.CHILD_CHANGED);
						ASTChangeEvent evt = new ASTChangeEvent(ASTChangeEvent.POST_UPDATE, 
								ASTChangeEvent.PROJECT_LEVEL, newNode, projectDelta);
						notifyListeners(evt);
						result = true;
					}
				}				
	...
	protected void updateProject(IProject project, IGlobalRootNode newNode) {
		...
		if (!fProjectASTMap.containsKey(project)) {
			...
			try {
				...
				IASTDelta[] childDelta = compareFileNodes(projectNode, oldFileNode, newFileNode);
				ASTDelta projectDelta = new ASTDelta(projectNode, childDelta);
	 ...
	 * @see updateProject
	 * !!This method is called within a locked context!!
	 * 
	 * This method will add new file nodes, note removed file nodes and
	 * update changed file nodes.
	 * 
	 * @param projectNode The project node for which file will be compared
	 * @param oldFileNode The old set of file nodes
	 * @param newFileNode The new set of file nodes
	...
	protected IASTDelta[] compareFileNodes(IGlobalRootNode projectNode, 
		...
		for (int i = 0; i < newFileNode.length; i++) {
			List<ILocalRootNode> nodeList = projectNode.lookupFileNode(newFileNode[i].getFile());
			boolean foundCorresponding = false;
			for (ILocalRootNode node : nodeList) {
				if (node.correspondsTo(newFileNode[i])) {
					...
					if (node.shouldBeUpdatedWith(newFileNode[i])) { 
						...
						ASTDelta delta = new ASTDelta(newFileNode[i], new IASTDelta[]{});
						delta.setStatus(IASTDelta.CHANGED|IASTDelta.CONTENT);
						deltaList.add(delta);
					}
				}
		...
		for (int i = 0; i < oldFileNode.length; i++) { 	
			...
			if (!foundCorresponding) {
				...
				ASTDelta delta = new ASTDelta(oldFileNode[i], new IASTDelta[]{});
				delta.setStatus(IASTDelta.REMOVED);
			}
		}
	 ...
	 * @param newNode The node of the project
	 */
	protected void registerProject(IProject project, IGlobalRootNode newNode) {
		// TODO: Lock?
		fProjectASTMap.put(project, newNode);
		ILocalRootNode[] fileNode = newNode.lookupAllFileNodes();
		ASTDelta[] fileDelta = new ASTDelta[fileNode.length];
		for (int i = 0; i < fileNode.length; i++) {
			fileDelta[i] = new ASTDelta(fileNode[i], new IASTDelta[]{});
			fileDelta[i].setStatus(IASTDelta.ADDED);
		}
		ASTDelta projectDelta = new ASTDelta(newNode, fileDelta);
	 ...
	 * @param evt The AST change event
	 */
	protected void notifyListeners(final IASTChangeEvent evt) {
		final IASTChangeListener[] l = new IASTChangeListener[fListereList.size()];
