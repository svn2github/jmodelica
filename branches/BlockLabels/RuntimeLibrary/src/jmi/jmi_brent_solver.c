...
int brentf(realtype y, realtype* f, void* problem_data) {
    ...
    if ((y- y) != 0) {
        jmi_log_node(block->log, logWarning, "Warning", "Not a number in arguments to <block: %s>", block->label);
        return -1;
    }
    ...
    /* Check that outputs are valid */    
    {
        realtype v = *f;
        if (v- v != 0) {
            jmi_log_node(block->log, logWarning, "Warning", "Not a number in output from <block: %s>", block->label);
            ret = 1;
        }
    }
    return ret;
    ...
    if ((f > DBL_MIN) || ((f < -DBL_MIN))) {
        ...
        while (1) {
            if (lower > block->min[0] && /* lower is fine as long as we're inside the bounds */
                ...
                jmi_log_node(log, logError, "Error", "Could not bracket the root in <block: %s>", block->label);
