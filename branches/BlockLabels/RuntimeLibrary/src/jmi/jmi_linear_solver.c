...
int jmi_linear_solver_solve(jmi_block_solver_t * block){
    ...
    if (solver->cached_jacobian != 1) {
        ...
        * */
        ...
        if(info) {
            if(block->init) {
                jmi_log_node(block->log, logError, "Error", "Singular Jacobian detected for <block: %s>", 
                             block->label);
            }
            else {
                jmi_log_node(block->log, logWarning, "Warning", "Singular Jacobian detected for <block: %s> at <t: %f>", 
