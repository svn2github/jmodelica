...
def test_ccpp_old():
    ...
    ccpp_old.run_demo(False)

@testattr(casadi = True)
def test_cstr_casadi_old():
    """Run the CSTR CasADi example."""
    cstr_casadi_old.run_demo(False)
...
@testattr(casadi = True)
def test_parameter_estimation_1_casadi_old():
    """Run the Parameter Estimation CasADi example."""
    parameter_estimation_1_casadi_old.run_demo(False)
    
@testattr(casadi = True)
def test_qt_par_est_casadi_old():
    """ Run quad tank parameter estimation CasADi example """
    qt_par_est_casadi_old.run_demo(False)
    
@testattr(casadi = True)
def test_vdp_casadi_old():
    """Run the VDP CasADi example."""
    vdp_casadi_old.run_demo(False)
