...
/**
 * The following tests are a bit unstable w.r.t changes in implementation, and new tests 
 * should be written in the style of the tests in {@link IndentedSectionTest} instead.
 */
static IndentTestCase[] testCases = { 
    ...
    /* 18 */           new IndentTestCase("model 'apaa", Indent.UNCHANGED, "apaa", "'apaa"),
    /* 19 */           new IndentTestCase("model 'apaa'\t", Indent.INDENT, "\t", "model '"),
    /* 20 */           new IndentTestCase("model 'ap\n\na\na\t\f'\t", Indent.INDENT, "\t", "model"),
    /* 21 */           new IndentTestCase("model 'apa/*not a comment", Indent.UNCHANGED, "apa", "'apa"),
    /* 22 */           new IndentTestCase("model \"teststr", Indent.UNCHANGED, "teststr", null),
    ...
    /* 42 */           new IndentTestCase("model m \n for i in 1:size(b,1) loop", Indent.INDENT, " i in", "for i"),
    /* 43 */           new IndentTestCase("model m \n for i in 1:size(b,1) loop\n result := 3;\t", Indent.SAME, "\t", "result"),
    /* 44 */           new IndentTestCase("model m \n for i in 1:size(b,1) loop\n result := 3;\nend for;\t", Indent.SAME, "\t", "for"),
    /* 45 */           new IndentTestCase("model m\n  /* comment */\n  Real r;\t", Indent.SAME, "\t", "Real"),
    /* 46 */           new IndentTestCase("model q\n  /* comment */ \n annotation ()\t", Indent.INDENT, "\t", "model"),
    /* 47 */           new IndentTestCase("model m\nReal r;equation", Indent.SAME, "equation", "Real"),
    /* 48 */           new IndentTestCase("model m\nReal r;\n\n\nequation\n", Indent.SAME, "\n\n\nequation", "Real"),
    /* 49 */           new IndentTestCase("model m model q end q end m; end z;\t", Indent.SAME, "\t", "model m"),  
    /* 50 */           new IndentTestCase("\nmodel m model q end q end m; end z;\t", Indent.SAME, "\t", "model m"),  
    /* 51 */           new IndentTestCase("model modelmodel", Indent.INDENT, " modelmodel", "model modelmodel"),  
       
        /* sink tests */
    /* 52 */    new IndentTestCase("model m\nend m;\t", Indent.SAME, "\t", "model m", "model m"),
    /* 53 */    new IndentTestCase("model m model q \nend q;\t", Indent.SAME, "\t", "model q", "model q"),
    /* 54 */    new IndentTestCase("/* a comment\n   */", null, "*/", "/* a", "/* a"),
    };
