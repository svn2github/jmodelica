...
public class EndOfBlockAdder extends EndStatementAdder {
	...
	final static String[] openBlockKeywords = { "block", "class", "connector", "function", "model",
			"package", "record", "type" };

	final static String NEWLINE = "(\r\n|\n|\r)\\s*";

	final static String classRegex = String.format("(.|\r|\n)*(^|\\s)(%s)\\s+\\w+(\\ |\t)*",
	...
	public void customizeDocumentCommand(IDocument doc, DocumentCommand c) {
		try {
			...
			if (!c.text.matches(NEWLINE)
					|| Util.is(doc.getPartition(c.offset).getType()).notAmong(
