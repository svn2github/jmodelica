...
public class OutlineChildWorker {
	
	private static OutlineChildCalcJob job = new OutlineChildCalcJob();
	...
	private static class OutlineChildCalcJob extends Job {
		
		private static Object lock = new Object();

		private static final String JOB_NAME = "Cind outline children";
		
		private Queue<IconInfo> queue = new LinkedList<IconInfo>();
		private boolean running = false;

		public OutlineChildCalcJob() {
			super(JOB_NAME);
			setSystem(true);
		}
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			while (running) {
				...
				if (info != null) {
					...
					new OutlineChildUpdateJob(info).schedule();
				}
				synchronized (lock) {
					running = !queue.isEmpty();
				}
	...
	private static class OutlineChildUpdateJob extends UIJob {

		private static final String JOB_NAME = "Update icons";
		
		private IconInfo info;

		public OutlineChildUpdateJob(IconInfo info) {
			super(JOB_NAME);
			setSystem(true);
