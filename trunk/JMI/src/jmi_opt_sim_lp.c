...
//	printf("<< %f\n",jmi_get_z(jmi_opt_sim->jmi)[0]);
	...
	if (jmi_opt_sim->jmi->opt->L->n_eq_F>0) {
		...
		for (i=0;i<jmi_opt_sim->n_e;i++) {
			...
			for (j=0;j<nlp->n_cp;j++) {
				...
				// Call cost function evaluation
				jmi_opt_L(jmi_opt_sim->jmi, &tmp_f);
				// Sum and multiply with weight times element length.
				*f += tmp_f*nlp->w[j]*jmi_opt_sim->hs[i]*
					  (jmi_opt_sim->jmi->opt->final_time-jmi_opt_sim->jmi->opt->start_time);
			}
		}
	}
...
//	printf("lp_radau_df\n");
	...
	if (jmi_opt_sim->jmi->opt->L->n_eq_F>0) {
		jmi_real_t *dp = (jmi_real_t*)calloc(jmi_opt_sim->jmi->opt->n_p_opt,
				sizeof(jmi_real_t));
		jmi_real_t w_sum = 0;
		...
		if (jmi_opt_sim->n_blocking_factors>0) {
				...
				for (j=0;j<nlp->n_cp;j++) {
					...
					if (j==0) {
						...
						jmi_opt_dL(jmi_opt_sim->jmi, nlp->der_eval_alg, JMI_DER_DENSE_COL_MAJOR,
								...
								df + offs_dx_coll(jmi_opt_sim,i,j+1,0));
						// Multiply with weights
						for (k=0;k<2*jmi_opt_sim->jmi->n_real_dx;k++) {
							df[offs_dx_coll(jmi_opt_sim,i,j+1,0) + k] *= jmi_opt_sim->hs[i]*
							nlp->w[j]*(jmi_opt_sim->jmi->opt->final_time-jmi_opt_sim->jmi->opt->start_time);
						}
						// Threat the us differently to compensate for
						// blocking factors
						for (k=0;k<jmi_opt_sim->jmi->n_real_u;k++) {
								df[offs_u_coll(jmi_opt_sim,i,j+1,0) + k] *= jmi_opt_sim->hs[i]*
								w_sum*(jmi_opt_sim->jmi->opt->final_time-jmi_opt_sim->jmi->opt->start_time);
						}
						for (k=0;k<jmi_opt_sim->jmi->n_real_w;k++) {
							df[offs_w_coll(jmi_opt_sim,i,j+1,0) + k] *= jmi_opt_sim->hs[i]*
							nlp->w[j]*(jmi_opt_sim->jmi->opt->final_time-jmi_opt_sim->jmi->opt->start_time);
						}
					} else {
						...
						for (k=0;k<2*jmi_opt_sim->jmi->n_real_dx +
							jmi_opt_sim->jmi->n_real_w;k++) {
							df[offs_dx_coll(jmi_opt_sim,i,j+1,0) + k] *= jmi_opt_sim->hs[i]*
								nlp->w[j]*(jmi_opt_sim->jmi->opt->final_time-jmi_opt_sim->jmi->opt->start_time);
						}
					}
			...
			for (i=0;i<jmi_opt_sim->n_e;i++) {
				for (j=0;j<nlp->n_cp;j++) {
					...
					// Multiply with weights
					for (k=0;k<2*jmi_opt_sim->jmi->n_real_dx +
						jmi_opt_sim->jmi->n_real_u + jmi_opt_sim->jmi->n_real_w;k++) {
						df[offs_dx_coll(jmi_opt_sim,i,j+1,0) + k] *= jmi_opt_sim->hs[i]*
							nlp->w[j]*(jmi_opt_sim->jmi->opt->final_time-jmi_opt_sim->jmi->opt->start_time);
					}
				}
...
static int lp_radau_get_result_mesh_interpolation(jmi_opt_sim_t *jmi_opt_sim,
	...
	for (i=0;i<n_mesh;i++) {
		...
		for (j=0;j<jmi_opt_sim->n_e;j++) {
			ti += jmi_opt_sim->hs[j];
			if (mesh_norm<=ti) {
				t_e[i] = j;
				t_tau[i] = (mesh_norm - (ti - jmi_opt_sim->hs[j]))/(jmi_opt_sim->hs[j]);
				break;
			}
			t_e[i] = jmi_opt_sim->n_e-1;
			t_tau[i] = 1.0;
		}
	}
	...
	for (i=0;i<n_mesh;i++) {
		...
		if (jmi->opt->final_time_free == 0 &&
				jmi->opt->start_time_free == 0 &&
				jmi_opt_sim->hs_free ==0) {
			el_length = jmi_opt_sim->hs[t_e[i]]*(jmi->opt->final_time - jmi->opt->start_time);
		} else { // TODO: Take care of the other cases
			el_length=0;
		}
		...
		for (k=0;k<jmi->n_real_dx;k++) {
			dx[i+k*n_mesh] = 0;
			for (l=0;l<nlp->n_cp+1;l++) {
				dx[i+k*n_mesh] += jmi_opt_sim_lp_eval_pol(t_tau[i],nlp->n_cp+1, nlp->Lpp_dot_coeffs, l)*
					x_coll(jmi_opt_sim,t_e[i],l,k)/el_length;
			}
		}
		...
		for (k=0;k<jmi->n_real_x;k++) {
			x[i+k*n_mesh] = 0;
			for (l=0;l<nlp->n_cp+1;l++) {
				x[i+k*n_mesh] += jmi_opt_sim_lp_eval_pol(t_tau[i],nlp->n_cp+1, nlp->Lpp_coeffs, l)*
					x_coll(jmi_opt_sim,t_e[i],l,k);
			}
		}
		...
		for (k=0;k<jmi->n_real_u;k++) {
			u[i+k*n_mesh] = 0;
			for (l=0;l<nlp->n_cp;l++) {
				u[i+k*n_mesh] += jmi_opt_sim_lp_eval_pol(t_tau[i],nlp->n_cp, nlp->Lp_coeffs, l)*
					u_coll(jmi_opt_sim,t_e[i],l+1,k);
			}
		}
		...
		for (k=0;k<jmi->n_real_w;k++) {
			w[i+k*n_mesh] = 0;
			for (l=0;l<nlp->n_cp;l++) {
				w[i+k*n_mesh] += jmi_opt_sim_lp_eval_pol(t_tau[i],nlp->n_cp, nlp->Lp_coeffs, l)*
					w_coll(jmi_opt_sim,t_e[i],l+1,k);
			}
		}
	}
...
int jmi_opt_sim_lp_new(jmi_opt_sim_t **jmi_opt_sim, jmi_t *jmi, int n_e,
	...
	*jmi_opt_sim = (jmi_opt_sim_t*)opt;
	...
	jmi_opt_sim_lp_get_pols(n_cp, opt->cp, opt->w,opt->cpp, opt->Lp_coeffs,
