...
model SimplifyCancellations4
    Integer[2] x = y .- 1;
    Integer[2] y = x .- x;
    
    annotation(__JModelica(UnitTesting(tests={
        FlatteningTestCase(
            name="SimplifyCancellations4",
            description="Simplifying cancellations of two equal expressions",
            flatModel="
fclass SymbolicTests.SimplifyCancellations4
 discrete Integer x[2] = y[1:2] .- 1;
 discrete Integer y[2] = zeros(2);
end SymbolicTests.SimplifyCancellations4;
")})));
end SimplifyCancellations4;
