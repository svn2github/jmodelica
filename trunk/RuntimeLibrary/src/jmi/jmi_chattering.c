...
void jmi_chattering_completed_integrator_step(jmi_t* jmi) {
    ...
    if (!jmi->chattering->chattering_detection_mode) {
        return;
    }
    
    jmi->chattering->clear_counter++;
    if (jmi->chattering->clear_counter >= CHATTERING_CLEARED_LIMIT) {
        for (i = 0; i < jmi->n_sw; i++) {
            jmi->chattering->chattering[i] = 0;
        }
        jmi->chattering->max_chattering = 0;
        jmi->chattering->logging_counter = 0;
        jmi->chattering->chattering_detection_mode = 0;
...
jmi_chattering_t* jmi_chattering_create(jmi_int_t n_sw) {
    ...
    chattering->pre_switches = calloc(n_sw, sizeof(jmi_int_t));
