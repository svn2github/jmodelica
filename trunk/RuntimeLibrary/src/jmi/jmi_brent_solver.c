...
int brentdf(realtype y, realtype* df, void* problem_data) {
    jmi_block_solver_t *block = (jmi_block_solver_t*)problem_data;
    int ret = 0;
    realtype y0 = y, yscale;
    realtype f, ftemp;
    int sign;
    realtype inc;
    ...
    yscale = 1.0/block->nominal[0];
    sign = (y >= 0) ? 1 : -1;
    inc = MAX(ABS(y), yscale)*sign*1e-8;
    y += inc;
