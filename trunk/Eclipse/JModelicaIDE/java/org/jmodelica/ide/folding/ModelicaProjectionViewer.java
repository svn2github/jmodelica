 ...
 */
public class ModelicaProjectionViewer extends SourceViewer implements ITextViewerExtension5 {
	...
	private ModelicaProjectionSupport fProjectionSupport;
	 ...
	 */
	public ModelicaProjectionViewer(Composite parent, IVerticalRuler ruler, IOverviewRuler overviewRuler, boolean showsAnnotationOverview, int styles) {
	...
	private void addProjectionAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			...
			extension.addAnnotationModel(ModelicaProjectionSupport.PROJECTION, fProjectionAnnotationModel);
	...
	private IAnnotationModel removeProjectionAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			...
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			return extension.removeAnnotationModel(ModelicaProjectionSupport.PROJECTION);
		}
		return null;
	}
	...
	public ProjectionAnnotationModel getProjectionAnnotationModel() {
		...
		if (model instanceof IAnnotationModelExtension) {
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			return (ProjectionAnnotationModel) extension.getAnnotationModel(ModelicaProjectionSupport.PROJECTION);
		}
		return null;
	}
	...
	private void expandAll(boolean modelica) {
		...
		while (iterator.hasNext()) {
			...
			if ((annotation instanceof AnnotationProjectionAnnotation) == modelica)
...
//				length= toLineStart(projection.getMasterDocument(), end, true) - offset;
	...
	public IRegion computeCollapsedRegion(Position position) {
		try {
			...
			int offset, length;
			if (position instanceof AnnotationPosition) {
