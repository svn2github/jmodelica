 ...
 *     Modeleon AB     - modification to support folds that aren't limited 
 *                       to line breaks
 *******************************************************************************/
 ...
 */
public class CharacterProjectionViewer extends SourceViewer implements ITextViewerExtension5 {
	...
	/** Operation constant for the expand all Modelica annotation folds operation. */
	public static final int EXPAND_ANNOTATIONS= BASE + 6;
	/** Operation constant for the collapse all Modelica annotation folds operation. */
	public static final int COLLAPSE_ANNOTATIONS= BASE + 7;
    ...
    /** Our ProjectionSupport - only needed so we can give it our LineBackgroundListener. */
	private CharacterProjectionSupport fProjectionSupport;
	/** Our LineBackgroundListener - only needed so we can give it to our ProjectionSupport. */
	private LineBackgroundListener fLineBackground;
	 ...
	 */
	public CharacterProjectionViewer(Composite parent, IVerticalRuler ruler, IOverviewRuler overviewRuler, boolean showsAnnotationOverview, int styles) {
	...
	private void addProjectionAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			...
			extension.addAnnotationModel(CharacterProjectionSupport.PROJECTION, fProjectionAnnotationModel);
	...
	private IAnnotationModel removeProjectionAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			...
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			return extension.removeAnnotationModel(CharacterProjectionSupport.PROJECTION);
		}
		return null;
	}
	...
	public ProjectionAnnotationModel getProjectionAnnotationModel() {
		...
		if (model instanceof IAnnotationModelExtension) {
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			return (ProjectionAnnotationModel) extension.getAnnotationModel(CharacterProjectionSupport.PROJECTION);
		}
		return null;
	}
	...
	private void expandAll(boolean modelica) {
		// We want to handle Modelica annotation folds separately
		Iterator iterator= fProjectionAnnotationModel.getAnnotationIterator();
		while (iterator.hasNext()) {
			Annotation annotation = (Annotation) iterator.next();
			if ((annotation instanceof CharacterProjectionAnnotation) == modelica)
	 ...
	 * @since 3.2
	 */
	private void collapseAll(boolean modelica) {
		// We want to handle Modelica annotation folds separately
		Iterator iterator= fProjectionAnnotationModel.getAnnotationIterator();
		while (iterator.hasNext()) {
			Annotation annotation = (Annotation) iterator.next();
			if ((annotation instanceof CharacterProjectionAnnotation) == modelica)
				fProjectionAnnotationModel.collapse(annotation);
		}
	}
	...
	private void addMasterDocumentRange(ProjectionDocument projection, int offset, int length) throws BadLocationException {
		...
		if (fCommandQueue != null) {
			...
			try {
				...
				// Removed to be able to add Modelica annotation folds that aren't line based 
				// TODO: need other fix for bug
...
//				length= toLineStart(projection.getMasterDocument(), end, true) - offset;
	...
	// TODO: remove? shouldn't be needed and should display uses that I missed
	/**
	...
	public IRegion computeCollapsedRegion(Position position) {
		try {
			...
			// If the fold is a Modelica annotation, don't force it to a line.
			int offset, length;
			if (position instanceof CharacterPosition) {
				offset = position.getOffset();
				length = position.getLength();
			} else {
				int line = document.getLineOfOffset(position.getOffset());
				offset = document.getLineOffset(line + 1);
				length = position.getLength() - (offset - position.getOffset());
			}
	...
	public IRegion modelRange2WidgetRange(IRegion modelRange) {
		IRegion res = super.modelRange2WidgetRange(modelRange);
		if (res == null && fInformationMapping instanceof IDocumentInformationMappingExtension2) {
			IDocumentInformationMappingExtension2 ext = (IDocumentInformationMappingExtension2) fInformationMapping;
			try {
				res = ext.toClosestImageRegion(modelRange);
				if (res.getLength() > 0)
					return null;
	...
	public void doOperation(int operation) {
		...
		switch (operation) {
			...
			case EXPAND_ANNOTATIONS:
				if (redraws())
					expandAll(true);
				break;

			case COLLAPSE_ANNOTATIONS:
				if (redraws())
					collapseAll(true);
				break;
	...
	// TODO: change javadoc comment - *character* based folding - should otherwise work
	/**
