...
void *fmimalloc(size_t size) {
    fmi_memory_header_t* header;
    fmi_callbacks_t* fmi_xml_c_b;
    jm_callbacks* jmCB = jm_get_default_callbacks();
    fmi_xml_c_b = jmCB->context;

    header = fmi_xml_c_b->fmiFunctions.allocateMemory(1, size + sizeof(fmi_memory_header_t) );
    if(!header) return 0;
    header->callbacks = fmi_xml_c_b;
    header->size = size;
    header++;
    return header;
}
...
void *fmicalloc(size_t nelements, size_t elementSize){
    ...
    */
    assert(!nelements || !elementSize || ((ULONG_MAX/elementSize) > (nelements +(sizeof(fmi_memory_header_t)+elementSize-1)/elementSize)));
...
void fmifree ( void * ptr ){
    fmi_memory_header_t* header = ptr;
    header--;
